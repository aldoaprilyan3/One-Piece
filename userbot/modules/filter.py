...
@register(outgoing=True, pattern=r"^.auto (.*)")
async def add_new_filter(new_handler):
    """ For .auto command, allows adding new filters in a chat """
    try:
    ...
    if msg and msg.media and not string:
        if BOTLOG_CHATID:
            await new_handler.client.send_message(
                ...
                "\n\n`Pesan Berikut Disimpan Sebagai Data Balasan Auto Untuk Obrolan, Harap Jangan Menghapusnya Sensei`"
            )
...
@register(outgoing=True, pattern=r"^.berhenti (.*)")
async def remove_a_filter(r_handler):
    ...
    if not remove_filter(r_handler.chat_id, filt):
        ...
        await r_handler.edit(
            "`Berhasil Menghapus Auto` **{}** `Disini`.".format(filt))
...
async def kick_marie_filter(event):
    ...
    await event.edit("```Sensei Saya Akan Menghapus Semua Auto!```")
    await sleep(3)
...
@register(outgoing=True, pattern="^.autod$")
...
CMD_HELP.update({
    "auto":
    "`.autod`\
    \nUsage: Melihat Auto/Filter One Piece yang aktif di obrolan ini.\
    \n\n`.auto` <keyword> <balasan> atau balas ke pesan ketik .auto <keyword>\
