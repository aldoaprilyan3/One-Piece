...
NO_ADMIN = "`Sensei Bukan Admin:)`"
NO_PERM = "`Sensei Tidak Mempunyai Izin!`"
NO_SQL = "`Berjalan Pada Mode Non-SQL`"

CHAT_PP_CHANGED = "`Berhasil Mengubah Profil Grup`"
CHAT_PP_ERROR = (
    "`Ada Masalah Dengan Memperbarui Foto,`"
    "`Mungkin Karna Sensei Bukan Admin,`"
...
async def set_group_photo(gpic):
    if not gpic.is_group:
        await gpic.edit("`Sensei Mohon Lakukan Perintah Ini Di Grup.`")
        return
    ...
    async for user in show.client.iter_participants(show.chat_id):
        if user.deleted:
            try:
                ...
                return await show.edit("`Sensei Tidak Memiliki Izin Banned Dalam Grup Ini`")
...
@register(outgoing=True, pattern=r"^\.adm$")
...
async def get_users(show):
    ...
    try:
        ...
        await show.edit("Sensei, Grup Ini Terlalu Besar Mengunggah Daftar Pengguna Sebagai File.")
...
async def get_usersdel(show):
    ...
    try:
        ...
        await show.edit(
            "Sensei, Grup Ini Terlalu Besar, Mengunggah Daftar Akun Terhapus Sebagai File."
        )
        file = open("daftarpengguna.txt", "w+")
        file.write(mentions)
        file.close()
...
async def get_bots(show):
    ...
    try:
        ...
        await show.edit("Sensei, Terlalu Banyak Bot Di Grup Ini, Mengunggah Daftar Bot Sebagai File.")
