...
async def igsaver(event):
    ...
    if not event.reply_to_msg_id:
        await event.edit("`Sensei, Mohon Reply Ke Link Instagramnya..`")
        return
    reply_message = await event.get_reply_message()
    if not reply_message.text:
        await event.edit("`Mohon Maaf Yang Sensei, Saya Membutuhkan Link Media Instagram Untuk di Download`")
        return
    chat = "@SaveAsBot"
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await event.edit("`Sensei,Tolong Pergi ke ` @SaveAsbot `Lalu Tekan Start dan Coba Lagi.`")
            return
